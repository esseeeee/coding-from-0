...
function countProfit(shoppers) {
  ...
  for (let i =0 ;i < listBarang.length;i++) {
    ...
    for (let j =0;j < shoppers.length;j++) {
      if (shoppers[j].product === productInfo.product && shoppers[j].amount <= productInfo.leftOver) {
        productInfo.shoppers.push(shoppers[j].name);
        productInfo.leftOver -= shoppers[j].amount;
        productInfo.totalProfit += shoppers[j].amount * listBarang[i][1];
      }
      // console.log (productInfo.totalProfit)
    }
    total.push(productInfo)
  }
  return total
}
